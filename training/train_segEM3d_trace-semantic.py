...
from training.utils.aftercare import visualize_and_save_mask, calculate_multi_class_voi
...
from utils.dataloader_ninanjie import load_train_dataset, \
    collate_fn_3D_ninanjie_Train, get_acc_prec_recall_f1, load_3d_semantic_dataset
...
def trainer_cellmask():
    ...
    log_dir = f'./output/log/segEM_3d_trace_semantic_cellmask/'
    ...
    with tqdm(total=training_epochs) as pbar:
        ...
        while epoch < training_epochs:
            ...
            for raw, cellmasked_raw, labels, mask, affinity, point_map in tmp_loader:
                ...
                model_step(model, optimizer, cellmasked_raw, point_map, labels)
            ...
            for raw, cellmasked_raw, labels, mask, affinity, point_map in tmp_val_loader:
                with torch.no_grad():
                    labels = torch.as_tensor(labels.squeeze(), dtype=torch.float32)
                    loss_value, pred = model_step(model, optimizer, cellmasked_raw, point_map, labels, train_step=False)
