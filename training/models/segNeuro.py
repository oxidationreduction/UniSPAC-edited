...
class affinity2d(torch.nn.Module):
    def __init__(
        self,
    ):
        super(affinity2d, self).__init__()
    ...
    def forward(self, x):
        y_affinity = self.affinity_predict(self.model_affinity(x))
        y_affinity = self.sigmoid(y_affinity)

        return y_affinity


class segneuro2d(torch.nn.Module):
    def __init__(
        self,
    ):
        super(segneuro2d, self).__init__()
        
        ##For affinity prediction
        self.model_affinity = affinity2d()
        model_path = './output/checkpoints/affinity2d(hemi+fib25)_Best_in_val.model' 
    ...
    def forward(self, x_raw, x_prompt):
        ...
        y_affinity = self.model_affinity(x_raw)
        ...
        return y_mask,y_affinity


 

class affinity3d(torch.nn.Module):
    def __init__(
        self,
    ):
        super(affinity3d, self).__init__()
...
class segneuro3d(torch.nn.Module):
    def __init__(
        self,
    ):
        super(segneuro3d, self).__init__()
        
        ##2D slice mask prediction
        self.model_mask_2d = segneuro2d()
        model_path = './output/checkpoints/segneuro2d(hemi+fib25)_Best_in_val.model'
        weights = torch.load(model_path,map_location=torch.device('cpu'))
        ...
        ##For affinity prediction
        self.model_affinity = affinity3d()
        model_path = './output/checkpoints/affinity3d(hemi+fib25)_Best_in_val.model' 
        weights = torch.load(model_path,map_location=torch.device('cpu'))
        self.model_affinity.load_state_dict(weights)
        for param in self.model_affinity.parameters():
            param.requires_grad = False
    ...
    def forward(self, x_raw, x_prompt):
        ...
        '''
        ##Get mask for slice0
        y_mask2d_slice0,_ = self.model_mask_2d(x_raw[:,:,:,:,0],x_prompt)
        ...
        return y_mask3d,y_mask2d_slice0,y_affinity
...
class segneuro3d_trace(torch.nn.Module):
    def __init__(
        self,
    ):
        super(segneuro3d_trace, self).__init__()
    ...
    def forward(self, x_raw, gt_mask2d_slice0):
        '''
        x_raw: shape = (Batch * channel=1 * dim_x * dim_y * dim_z)
