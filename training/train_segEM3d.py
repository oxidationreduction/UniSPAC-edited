...
class segEM_3d(torch.nn.Module):
    def __init__(
        ...
        # model_path = './output/checkpoints/segEM2d(hemi+fib25)faster_wloss3({})_Best_in_val.model'.format(WEIGHT_LOSS3)
...
if __name__ == '__main__':
    ...
    # Save_Name = 'segEM_3d(hemi+fib25)faster_wloss3({})'.format(WEIGHT_LOSS3)
    Save_Name = 'segEM_3d(hemi+fib25)'
    ...
    with tqdm(total=training_epochs) as pbar:
        while epoch < training_epochs:
            ...
            for raw, labels,mask,affinity,point_map in tmp_loader:
                ...
                loss_value, pred = model_step(model, optimizer, raw, point_map, mask, affinity, activation, train_step=True)
            ...
            for raw, labels,mask,affinity,point_map  in tmp_val_loader:
                ...
                with torch.no_grad():
                    loss_value, _ = model_step(model, optimizer, raw, point_map, mask, affinity, activation, train_step=False)
