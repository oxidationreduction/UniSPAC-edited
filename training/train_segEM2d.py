...
if __name__ == '__main__':
    ...
    ##装载数据
    train_dataset_1 = Dataset_2D_hemi_Train(data_dir='./data/funke/hemi/training/', split='train', crop_size=128, require_lsd=False,require_xz_yz=True)
    val_dataset_1 = Dataset_2D_hemi_Train(data_dir='./data/funke/hemi/training/', split='val', crop_size=128, require_lsd=False,require_xz_yz=True)
    
    train_dataset_2 = Dataset_2D_fib25_Train(data_dir='./data/funke/fib25/training/', split='train', crop_size=128, require_lsd=False,require_xz_yz=True)
    val_dataset_2 = Dataset_2D_fib25_Train(data_dir='./data/funke/fib25/training/', split='val', crop_size=128, require_lsd=False,require_xz_yz=True)
    ...
    with tqdm(total=training_epochs) as pbar:
        while epoch < training_epochs:
            ...
            for raw, labels,point_map,mask,gt_affinity in tmp_loader:
                ...
                loss_value, pred = model_step(model, optimizer, raw, point_map, mask , gt_affinity, activation, train_step=True)
            epoch += 1
            pbar.update(1)
            ...
            for raw, labels,point_map,mask,gt_affinity in tmp_val_loader:
                raw = torch.as_tensor(raw,dtype=torch.float, device= device) #(batch, 1, height, width)
                point_map = torch.as_tensor(point_map, dtype=torch.float, device=device) #(batch, 6, height, width)
                mask = torch.as_tensor(mask, dtype=torch.float, device=device) #(batch, 2, height, width)
                gt_affinity = torch.as_tensor(gt_affinity, dtype=torch.float, device=device) #(batch, 2, height, width)
                ...
                with torch.no_grad():
                    loss_value, _ = model_step(model, optimizer, raw, point_map, mask, gt_affinity, activation, train_step=False)
